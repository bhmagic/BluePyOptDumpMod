...
def get_log_file_id( input_dir):
    ...
    for files in file_list:
        cell_id = files.split('/')[-1].split('_sta')[0]
        stage_id = files.split('/')[-1].split('_')[-4]
        iteration_id = files.split('/')[-1].split('_')[-2]
...
#    for ii in range(int(command_line_input_index[1]), int(command_line_input_index[2])):
    file_id = get_file_id(['0', command_line_input_index[1], command_line_input_index[2] ], input_dir)
    output_dir = file_id+'_seed_'+ command_line_input_index[2] +'_Outputs'
